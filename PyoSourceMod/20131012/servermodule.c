...
Server_embedded_i_start(Server *self)
{
    ...
    /* set as stopped since only processing one buffer at every callback */
//     self->server_started = 0;
//     self->server_stopped = 1;
    return 0;
}
...
PyServer_get_server()
{
    return (PyObject *)my_server[serverID];     /* gbAdd */
}
...
{  
    ...
    my_server[self->thisServerID] = NULL;   /* gbAdd */
...
static PyMethodDef Server_methods[] = {
    ...
    //{"boot", (PyCFunction)Server_boot, METH_NOARGS, "Setup and boot the server."},    // gbAdd
